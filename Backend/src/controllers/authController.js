...
export const login = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Login Success", success: true, jwtToken, email: user.email, name: user.name, role: user.role, hasRD: user.hasRD, hasFD: user.hasFD, userImage: user.userImage, userSignature: user.userSignature})
    } catch (err) {
        res.status(500).json({message: "Internal server errror",success: false})
    }
}